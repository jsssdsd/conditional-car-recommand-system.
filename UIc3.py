    ...
    def final():
        ...
        try:
            ...
            if sc.wealthrangex < 2400:
                ResultViewlabel_ViewLabel.config(text="그 재력으로 뭘산다구요? 뚜벅이는 뚜벅이답게 행동하세요.")
            elif sc.wealthrangex > 13400:
                ResultViewlabel_ViewLabel.config(text="그 정도 재력이면 아무거나 사도 되니 대신 저나 좀 사주세요.")
            else:
        ...
        try:
            ...
            def loancal():
                def loaninp1():
                    global a
                    a = loantxt1.get()
                    loanbl1.config(text=loantxt1.get())  # loantxt1에 저장한다 입력값을  txt  loanbl라벨 .config로 텍스트상자 합침
                def loaninp2():
                    global b
                    b = loantxt2.get()
                    loanbl2.config(text=loantxt2.get())
                def loanfC1():
                    loanfC1.config(text="1개월 지불금액 : %s원" % bl.carLone)
                loancal = Toplevel(pro2)
                loancal.title("자동차 대출금 계산기")
                loancal.geometry("600x770")  # 창의 사이즈
                loancal.resizable(True, True)

                # framelt = tk.Frame(loancal, bd=4, bg='red')
                # framelt.grid(row=0, column=0)
                # framelb = tk.Frame(loancal)
                # framelb.grid(row=0, column=1)
                # framelc = tk.Frame(loancal)
                # framelc.grid(row=5,column=1)
                def closel():
                    loancal.destroy()
                bt1 = tk.Button(loancal, text="닫기", command=closel)
                bt1.grid(row=4, column=1)

                # listbox = Listbox(loancal)  # root라는 창에 리스트 박스 생성
                # listbox.config(selectmode="extended")  # 리스트 박스 selectmode설정 "extended" = 여러개 선택 가능
                # listbox.config(height=10)  # 리스트 박스  높이 설정
                # listbox.grid(row=1, column=3)
                # listbox.insert(1, "3개월")  # 첫번째 자리에 "파이썬"을 추가
                # listbox.insert(2, "6개월")  # 두번째 자리에 "C"을 추가
                # listbox.insert(3, "12개월")
                # listbox.insert(4, "18개월")
                # listbox.insert(5, "24개월")
                # listbox.insert(6, "30개월")
                # listbox.insert(7, "36개월")
                # listbox.insert(8, "48개월")
                # listbox.insert(9, "60개월")
                # listbox.insert(10, "72개월")
                # listbox.insert(11, "84개월")
                # listbox.insert(12, "96개월")
                # listbox.insert(13, "108개월")
                # listbox.insert(14, "120개월")
                # listbox.insert(END, "직접입력")  # 끝자리에 "java"을 추가
                # a = listbox.curselection()

                loanbl1 = Label(loancal, text="할부 원금")
                loanbl1.grid(row=1, column=0)
                loantxt1 = Entry(loancal)
                loantxt1.grid(row=1, column=1)
                loantxt1.bind("<Return>", loaninp1)

                loanbl2 = Label(loancal, text="선수금")
                loanbl2.grid(row=2, column=0)
                loantxt2 = Entry(loancal)  # Entry  텍스트창을 만든다. 2행 1열에   ,리턴값 을 laoan ip2함수를 불러와서 라벨 , 텍스트값을 출력  텍스트값은 함수의 정의에따라  b2에 저장한다
                loantxt2.grid(row=2, column=1)
                loantxt2.bind("<Return>", loaninp2)

                btnloanCcal = tk.Button(loancal, text="계산하기", command=loanfC)
                btnloanCcal.grid(row=1, column=2)
                labinsu = tk.Label(loancal)
                labinsu.grid(row=2, column=2, sticky="w")

                # loanbl = Label(loancal, text="만 나이")
                # loanbl.grid(row=0, column=0)
                # loantxt1 = Entry(loancal)
                # loantxt1.grid(row=0, column=1)
                # loantxt1.bind("<Return>", loaninp1)
                # loancal.mainloop()
                # pro.mainloop()
            ...
            james.info()
            def loancal():
                def loaninp1():  #할부 원금 입력값
                    global a
                    a = loantxt1.get()
                    loanbl1.config(text=loantxt1.get())  # loantxt1에 저장한다 입력값을  txt  loanbl라벨 .config로 텍스트상자 합침
                def loaninp2(): #선수금 입력값
                    global b
                    b = loantxt2.get()
                    loanbl2.config(text=loantxt2.get())
                ...
                def loaninp5(): #선수금 캐시백 입력값
                    global e
                    e = loantxt5.get()
                    loanbl5.config(text=loantxt5.get())
                def loanfC():
                    loanfC.config(text="1개월 지불금액 : %s원" % bl.carLone)
                loancal = Toplevel(pro2)
                loancal.title("자동차 대출금 계산기")
                loancal.geometry("600x770")  # 창의 사이즈
                loancal.resizable(True, True)
                ...
                def closel():
                    loancal.destroy()
                bt1 = tk.Button(loancal, text="대출창닫기", command=closel)
                ...
                a = [3, 6, 9,12,18,24,30,36,48,60,72,84,96,108,120,"직접입력"]  # 콤보 박스에 나타낼 항목 리스트
                combobox1 = ttk.Combobox(loancal)  # root라는 창에 콤보박스 생성
                combobox1.config(height=5)  # 높이 설정
                combobox1.config(values=a ) # 나타낼 항목 리스트(a) 설정
                combobox1.config(state="readonly")  # 콤보 박스에 사용자가 직접 입력 불가
                combobox1.set("할부기간(개월)을 선택하세요")  # 맨 처음 나타낼 값 설정
                combobox1.grid(row=1, column=3)  # 콤보 박스 배치
                loanmonthT = Label(loancal, text="개월 (할부기간)")
                loanmonthT.grid(row=1, column=4)

                b = [3, 6, 9, 12]  # 콤보 박스에 나타낼 항목 리스트
                combobox2 = ttk.Combobox(loancal)  # root라는 창에 콤보박스 생성
                combobox2.config(height=5)  # 높이 설정
                combobox2.config(values=b)  # 나타낼 항목 리스트(a) 설정
                combobox2.config(state="readonly")  # 콤보 박스에 사용자가 직접 입력 불가
                combobox2.set("거치 기간")  # 맨 처음 나타낼 값 설정
                combobox2.grid(row=2, column=3)  # 콤보 박스 배치
                loanmonthT = Label(loancal, text="개월 (거치기간)")
                loanmonthT.grid(row=2, column=4)

                c = [5, 10, 15, 20,25,30,35,40,45,50,55,60,65,70]  # 콤보 박스에 나타낼 항목 리스트
                combobox3 = ttk.Combobox(loancal)  # root라는 창에 콤보박스 생성
                combobox3.config(height=5)  # 높이 설정
                combobox3.config(values=c)  # 나타낼 항목 리스트(a) 설정
                combobox3.config(state="readonly")  # 콤보 박스에 사용자가 직접 입력 불가
                combobox3.set("사은품")  # 맨 처음 나타낼 값 설정
                combobox3.grid(row=3, column=3)  # 콤보 박스 배치
                loanmonthT = Label(loancal, text="만원 (사은품)")
                loanmonthT.grid(row=3, column=4)

                # loancal.QtCore.QString(u'Test Selection2')
                ...
                loanbl3.grid(row=3, column=0)
                loantxt3 = Entry(loancal)
                loantxt3.grid(row=3, column=1)
                loantxt3.bind("<Return>", loaninp3)

                loanbl4 = Label(loancal, text="원금 캐시백 (%)")
                loanbl4.grid(row=4, column=0)
                loantxt4 = Entry(loancal)
                loantxt4.grid(row=4, column=1)
                loantxt4.bind("<Return>", loaninp4)

                loanbl5 = Label(loancal, text="선수금 캐시백 (%)")
                loanbl5.grid(row=5, column=0)
                loantxt5 = Entry(loancal)
                loantxt5.grid(row=5, column=1)
                loantxt5.bind("<Return>", loaninp5)
                def loanFC():
                    lFC = Toplevel(pro2)
                    lFC.title("대출금 계산" )
                    lFC.geometry("500x600")  # 창의 사이즈
                    ...
                    loancal.mainloop()
                btnloanCcal = tk.Button(loancal, text="계산하기", command=loanFC)
                btnloanCcal.grid(row=6, column=2)
...
def nextpage2():  #  여기에 내차 견적 만들기!
    ...
    # ResultViewlabel_ViewLabel = tk.Label(frame4, text="차량이름: %s" % sc.cleandatareturn(),wraplength=500)  #차량 이름메뉴 출력.
    # ResultViewlabel_ViewLabel.grid(row=1, column=3, sticky="w", padx=5)   #차량 이름메뉴 출력.2

    # for i in range(len(cleancaridex())) :
    #     label_info0 = tk.Label(frame4, text="%s" % str(i[i]))
    #     label_info0.pack
    ...
    def final():
        ...
        if len(escar)==0:
            ...
            global a
            global e
            # a = command = ce.taxcal(txt1.get(), int(txt5.get()))
            esw1 = Toplevel(pro)
            esw1.title("견적 출력")
            ...
            frame0 = tk.Frame(esw1, bd=1, bg='yellow')
            frame0.pack(side='top')
            a =ce.taxcal(txt1.get(), int(txt5.get()))
            # a = ce.taxcal(a, int(e))
            print("=" * 100, escar)
            ...
            label_info3 = tk.Label(frame0, text="가격(단위:만):%s" % escar[1][2])
            ...
            def clear():
                # global frame0
                mylist = esw1.grid_slaves()
                for i in mylist:
                    i.destroy()
                esw1.destroy()
            def clearBot():
                mylist = frame0.grid_slaves()
                for i in mylist:
                    i.destroy()
                esw1.destroy()
            def closematch():
            ...
            # def closel():
            #     # final()
            #     for widget in esw1.winfo_children():
            #         widget.destroy()
...
def nextpage3():
    def result():
        ...
        testpic = tk.PhotoImage(file="./3조/차량사진/기아모닝.png")
        lbtestpic = tk.Label(testnext, image=testpic)
        lbtestpic.pack(side=TOP)
        ...
        if score == 0:
            ...
            lbfive.config(text="누가봐도 너무 착한 당신! 억지로 끼어드려는 차량까지 왠만하면 다 양보해주는 편이네요. 하지만 양보했는데 상대차량이 "
                               "고마움의 표시도 없이 숭~지나가면 속상해 합니다. 난폭운전을 하는 운전자를 만나도 싸움 나기 싫어 피하는 당신, 그대신 블랙박스를 돌려 스마트국민제보로 신고하는 편입니다. 내가 당했을 때보다 남이 당했을 때 더 분한 당신! 오지랖이 넓다는 소리도 듣지만 선의의 도움을 주는 착한 사람이군요!")
        elif score == 1:
            ...
            lbone.config(text="확고한 나만의 세계, 마이웨이")
            lbblank.config(text="")
            lbtwo.config(text="어울리는 차 : %s" % recom_car("준증형 해치백"))
            lbthree.config(text="어울리는 옵션 : 나만 아는 음악")
            lbfour.config(text="어울리는 도로 : 방글 깔린 아스팔트 위")
            lbfive.config(
...
def nextpage4():
    ...
    labeltext = tk.Label(frameone, text="나만의 차를 만들어보세요!", font=("Helvetica", 36, "bold"))
    labeltext.grid(row=0, column=0)
    btn1 = tk.Button(frametwo, width=29, height=10, text="스포츠카", font=("Helvetica", 15, "bold"), fg="black",
                     bg="orange", command=SportCar)
    btn1.grid(row=3, column=3)
    btn2 = tk.Button(frametwo, width=29, height=10, text="세단", font=("Helvetica", 15, "bold"), fg="black", bg="pink",
                     command=Sedan)
    btn2.grid(row=3, column=4)
    btn3 = tk.Button(frametwo, width=29, height=10, text="SUV", font=("Helvetica", 15, "bold"), fg="black",
                     bg="skyblue", command=SUV)
    btn3.grid(row=3, column=5)
    w0.mainloop()

btn1=tk.Button(win,width=25,height=10,text="차량 추천",font=("bold",10,"bold"),fg="black",bg="yellow",command=nextpage1)
btn1.grid(row=3, column=3)
btn2=tk.Button(win,width=25,height=10,text="내차 견적",font=("bold",10,"bold"),fg="white",bg="green", command=nextpage2)
btn2.grid(row=3, column=4)
btn3=tk.Button(win,width=25,height=10,text="운전성향 테스트",font=("bold",10,"bold"),fg="white",bg="blue",command=nextpage3)
btn3.grid(row=3, column=5)
btn4=tk.Button(win,width=25,height=10,text="나만의 차 만들기",font=("bold",10,"bold"),fg="white",bg="purple",command=nextpage4)
