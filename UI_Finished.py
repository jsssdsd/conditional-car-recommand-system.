...
#
#         # 동적 이미지 추가
#         self.movie = QMovie('./3조/asd/f3.gif', QByteArray(), self)
                ...
                def loanFC():
                    try:
                        ...
                        if (int(myc1[1][2]) / 2) < int(loantxt4.get()):
                            lastloanexc.config(text="원금캐시백이 한도(50%)를 초과했습니다.")
                            lFC.withdraw()
                        else:
...
btn1=tk.Button(MFrame,width=25,height=10,text="차량 추천",font=("bold",10,"bold"),fg="midnightblue",bg="lightblue",command=nextpage1)
btn1.grid(row=3, column=3)
btn2=tk.Button(MFrame,width=25,height=10,text="내차 견적",font=("bold",10,"bold"),fg="darkslategrey",bg="mediumaquamarine", command=nextpage2)
btn2.grid(row=3, column=4)
btn3=tk.Button(MFrame,width=25,height=10,text="운전성향 테스트",font=("bold",10,"bold"),fg="white",bg="cadetblue",command=nextpage3)
btn3.grid(row=3, column=5)
btn4=tk.Button(MFrame,width=25,height=10,text="나만의 차 만들기",font=("bold",10,"bold"),fg="indigo",bg="thistle",command=nextpage4)
btn4.grid(row=3, column=6)
photomain1 = tk.PhotoImage(file="./3조/asd/f3.gif")
labelmain1 = tk.Label(frameB, image=photomain1)
labelmain1.grid(row=0, column=0)
labeltext = tk.Label(frameB, text="__decorating my car__", font=("Courier", 36, "italic"), bg="tan")
